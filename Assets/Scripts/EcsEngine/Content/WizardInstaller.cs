...
namespace Content
{
    public class WizardInstaller : EntityInstaller
    {
        ...
        {
            //Logic:
            entity.AddData(new MoveSpeed{value = stats.GetStat(StatsData.WIZARD_SPEED)});
            entity.AddData(new Health{value = stats.GetStat(StatsData.WIZARD_HEALTH)});
            entity.AddData(new Damage{value = stats.GetStat(StatsData.WIZARD_POWER)});
